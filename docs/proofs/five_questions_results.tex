...
\begin{abstract}
We investigate five questions concerning the value sets of reversed Dickson polynomials $D_n(a,x)$ over $\mathbb{F}_p$ for primes $p > 3$.  We prove that the cardinality of the value set is~$1$ if and only if $n \equiv 0, 1,$ or $p \pmod{p^2-1}$, and that for nonzero parameter~$a$ the cardinality is independent of~$a$.  We also present computational observations on missing cardinalities, index patterns for higher cardinalities, and the special role of the primes $p = 5, 7, 11$.
\end{abstract}
...
\noindent\textbf{Notation convention.} Throughout we use reversed Dickson notation $D_n(a,x)$, with canonical case $D_n(1,x)$, defined exactly by $D_0(a,x)=2$, $D_1(a,x)=a$, and $D_n(a,x)=aD_{n-1}(a,x)-xD_{n-2}(a,x)$ for $n\ge2$.

\begin{definition}
Let $p > 3$ be a prime. The \emph{reversed Dickson polynomial} $D_n(a, x)$ over $\mathbb{F}_p$ is defined by the recurrence
\begin{align}
D_0(a, x) &= 2, \label{eq:base0}\\
D_1(a, x) &= a, \label{eq:base1}\\
D_n(a, x) &= a \cdot D_{n-1}(a, x) - x \cdot D_{n-2}(a, x) \quad (n \ge 2), \label{eq:recurrence}
\end{align}
...
\begin{definition}
The \emph{value set} of $D_n(a, x)$ is
\[
V_n(a) = \{D_n(a, x) : x \in \mathbb{F}_p\},
\]
...
Thus, as functions on $\mathbb{F}_p$, the polynomials $D_n(1,x)$ depend only on $n \bmod (p^2 - 1)$.
...
\[
D_n(a, x) = a^n \cdot D_n\!\left(\frac{x}{a^2},\, 1\right).
\]
...
\begin{align*}
D_0(a, x) &= 2 = a^0 \cdot 2 = a^0 \cdot D_0\!\left(\tfrac{x}{a^2}, 1\right). \\
D_1(a, x) &= a = a^1 \cdot 1 = a^1 \cdot D_1\!\left(\tfrac{x}{a^2}, 1\right).
\end{align*}

\noindent\textbf{Inductive step.}  Assume the identity holds for $n-1$ and $n-2$.  Then
\begin{align*}
D_n(a, x) &= a \cdot D_{n-1}(a, x) - x \cdot D_{n-2}(a, x) \\
